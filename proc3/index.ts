...
async function processInput(): Promise<number> {
  ...
  try {
    ...
    logger.info(`Result: ${outp.Result}`, {
      'dd.trace_id': calcSpan.context().toTraceId(),
      'dd.span_id': calcSpan.context().toSpanId(),
    });
    ...
    if (error instanceof z.ZodError) {
      ...
      return 2; // Return non-zero status code for validation error
    } else if (error instanceof SyntaxError) {
      logger.error('Failed to parse JSON', { error: error.message });
      return 3; // Return non-zero status code for JSON parsing exception
    } else {
      logger.error('An unexpected error occurred', { error: error instanceof Error ? error.message : String(error) });
      return 4; // Return non-zero status code for unexpected exceptions
    }
  }
